...
app.post('/api/ai-adventure/start', async (req, res) => {
    ...
    try {
        ...
        gameRooms[roomCode] = { history: [ { role: "user", parts: [{ text: systemPrompt }] }, { role: "model", parts: [{ text: result.response.text() }] } ] };
...
app.post('/api/ai-adventure/action', async (req, res) => {
    ...
    try {
        ...
        gameRooms[roomCode].history.push({ role: "model", parts: [{ text: result.response.text() }] });
        res.json({ success: true, message: result.response.text() });
    } catch (error) {
        console.error(error);
